...
fetch('/api/space/favorites?user_id='+id)
    .then(response => {
        if (response.ok) {
            return response.json(); // 解析JSON格式的响应数据
        }
    ...
    .then(data => {
        ...
        fetch('/api/space/favorites?user_id='+id+"&page="+now_page+"&per_page=20")
            .then(response =>{
                if (response.ok) {
                    return response.json(); // 解析JSON格式的响应数据
                }
...
const last_page = () =>{
    ...
    window.location.href = "/space/favorites?page="+x+"?id="+String(id);
    if (Number(page_num.value)>1){
        document.getElementById("front-page-prev").className = "page-item";
    }else{
        document.getElementById("front-page-prev").className = "page-item disabled";
