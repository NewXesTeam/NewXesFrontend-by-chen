...
var num,max;
fetch('/api/space/fans?user_id='+id)
    .then(response =>{
        if (response.ok) {
            return response.json(); // 解析JSON格式的响应数据
        }
    ...
    .then(data => {
        ...
        fetch('/api/space/fans?user_id='+id+"&page="+now_page)
            .then(response =>{
                if (response.ok) {
                    return response.json(); // 解析JSON格式的响应数据
                }
            ...
            .then(data => {
                ...
                //console.log(window.location.href.split("?")[1].split("page=")[1]);
...
const last_page = () =>{
    ...
    window.location.href = "/space/fans?page="+x+"?id="+String(id);
    if (Number(page_num.value)>1){
        document.getElementById("front-page-prev").className = "page-item";
    }else{
        document.getElementById("front-page-prev").className = "page-item disabled";
